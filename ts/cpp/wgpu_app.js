...
function findWasmBinary() {
  if (Module['locateFile']) {
    return locateFile('wgpu_app.wasm');
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('wgpu_app.wasm', import.meta.url).href;
}
...
var _load_gltf_buffer = Module['_load_gltf_buffer'] = createExportWrapper('load_gltf_buffer', 2);
...
(() => {
  // Create a small, never-async wrapper around CPPApp which
  // checks for callers incorrectly using it with `new`.
  var real_CPPApp = CPPApp;
  CPPApp = function(arg) {
    if (new.target) throw new Error("CPPApp() should not be called with `new CPPApp()`");
    return real_CPPApp(arg);
  }
})();
